...
function detectVerdict(output: string): string {
  // Prefer an explicit marker so free-form agent prose doesn't trigger
  // false positives (e.g. "cotação bloqueada até confirmação" ≠ BLOCKED verdict).
  // Accepted: "VERDICT: PASS", "## Verdict: BLOCKED", "**VERDICT:** NO-GO", etc.
  const markerMatch = output.match(
    /verdict\s*[:：]\s*\*?\*?\s*(approve|aprovad\w*|reject|rejeitad\w*|blocked|bloquead\w*|pass|passou|fail|falhou|no[-\s]?go|go|success)/i
  );
  if (markerMatch) {
    const token = markerMatch[1].toLowerCase();
    if (token.startsWith('approv') || token.startsWith('aprovad')) return 'APPROVE';
    if (token.startsWith('reject') || token.startsWith('rejeitad')) return 'REJECT';
    if (token.startsWith('block') || token.startsWith('bloquead')) return 'BLOCKED';
    if (token === 'pass' || token === 'passou') return 'PASS';
    if (token === 'fail' || token === 'falhou') return 'FAIL';
    if (/^no[-\s]?go$/.test(token)) return 'NO-GO';
    if (token === 'go') return 'GO';
    if (token === 'success') return 'SUCCESS';
  }

  // No explicit marker: any non-empty output counts as SUCCESS so the
  // sequence can progress. Callers gate progression on job.status === 'done'
  // anyway, so this keeps free-form prose from blocking valid runs.
