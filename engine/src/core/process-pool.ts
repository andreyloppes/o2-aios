...
async function spawnJob(slot: PoolSlot, job: Job): Promise<void> {
  ...
  try {
    ...
    const preUpdateJob = queue.getJob(job.id);
    if (preUpdateJob && preUpdateJob.status === 'running') {
      if (exitCode === 0) {
        ...
        // Auto-retry
        const canRetry = preUpdateJob.attempts < preUpdateJob.max_attempts;
        if (canRetry) {
          queue.updateStatus(job.id, 'pending');
          log.info('Job auto-retried', { jobId: job.id, attempt: preUpdateJob.attempts });
        }
      }
    }

    // Phase 7: Completion handler (memories, metrics, callbacks, cleanup)
    // Re-read job so downstream handlers (workflow-engine) see the terminal status
    const finalJob = queue.getJob(job.id) ?? preUpdateJob ?? job;
